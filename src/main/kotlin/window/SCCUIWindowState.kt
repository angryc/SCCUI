...
class SCCUIWindowState(
    ...
    var buttonColor = mutableStateListOf(Color.Green, Color.DarkGray, Color.DarkGray, Color.DarkGray, Color.DarkGray, Color.DarkGray, Color.DarkGray, Color.DarkGray, Color.DarkGray)

    var remapblock = mutableStateListOf("","","","","","","","","")
    ...
    fun updateRemapblock() {
        var i = 0
        remapblock[layer] = "remapblock \r\n"+"layer "+layer.toString()+"\r\n"
        rows.forEach {
            rows[i].forEach {
                if (it.mapTo[layer] != null) {
                    remapblock[layer] += "  " + it.name + " " + it.mapTo[layer] + "\r\n"
                }
            }
            i++
        }
        remapblock[layer] += "endblock"
        updateOutput()
    }
