...
class SCCUIWindowState(
    ...
    // temp storage until "apply" is pressed
    var macroNameTemp by mutableStateOf("")
    var triggerKeyTemp by mutableStateOf("")
    var triggerKeyDescriptionTemp by mutableStateOf("")
    var metaTriggersTemp = mutableStateListOf("", "", "", "")
    var actionTemp = mutableStateListOf("")
    var actionDescriptionTemp = mutableStateListOf("")
    var actionKeyTemp = mutableStateListOf("")
    ...
    fun saveMacroButtonPressed() {
        if (triggerKeyTemp != "" && triggerKeyTemp != "NOMAPPING") {
            ...
            for (i in 0 until actionTemp.size) {
                macros[selectedMacro].actions.add(MacroAction(actionKeyTemp[i], actionTemp[i], i))
            }
